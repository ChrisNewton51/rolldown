...
public class PlayerController : NetworkBehaviour
{
    ...
    IEnumerator Invincible()
    {
        ...
        Material disableMaterial = Resources.Load("Player Disabled", typeof(Material)) as Material;
        invincible = true;
        float waitTime = 0.15f;
        for (int i = 0; i < 6; i++)
        {
            gameObject.GetComponent<Renderer>().material.SetColor("_EmissionColor", emColor * 0.5f);
            gameObject.GetComponent<Renderer>().material.SetColor("_BaseColor", new Color(material.color.r - 20, material.color.g - 20, material.color.b - 20, 0.2f));
            yield return new WaitForSeconds(waitTime);
            gameObject.GetComponent<Renderer>().material.SetColor("_EmissionColor", emColor * 1);
